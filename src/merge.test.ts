...
describe("merge function", () => {
    ...
    test("handles some empty collections", () => {
        const collection_1: number[] = [2, 6, 10];
        const collection_2: number[] = [];
    ...
    test("merges when some arrays are negative numbers", () => {
        const collection_1: number[] = [-5, -4, -3];
        const collection_2: number[] = [-2, -1, 0];
        const collection_3: number[] = [3, 2, 1];
        const result = merge(collection_1, collection_2, collection_3);
        expect(result).toEqual([-5, -4, -3, -2, -1, 0, 1, 2, 3]);
    });

    test("merges when all arrays are negative numbers", () => {
        const collection_1: number[] = [-9, -4, -3];
        const collection_2: number[] = [-6, -5, -2];
        const collection_3: number[] = [-1, -7, -8];
        const result = merge(collection_1, collection_2, collection_3);
